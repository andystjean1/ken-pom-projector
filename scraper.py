...
def map_name_to_kenpom(name_tr: str) -> str:
    ...
    if(name_kp.endswith("St")): #ken pom has a period at the end of the state abbreviation
        name_kp = name_kp + '.'
...
#scrape the games from team rankings
def scrape_games_tourney(driver, url):
    driver.get(url)
    time.sleep(5)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
